...
def generate_disk_mesh(
    radius, size_min, size_max, sc_boundary=0.0, filename="disk.msh", gui=False
):
    gmsh.initialize()
    gmsh.clear()
    gmsh.model.add("disk")
    ...
    sc_dimtags = []
    if sc_boundary > 0.0:
        sc_raw = gmsh.model.occ.addDisk(
            0, 0, 0, radius + sc_boundary, radius + sc_boundary
        )
        sc_dimtags, _ = gmsh.model.occ.cut([(2, sc_raw)], [(2, disk)], -1, True, False)

    gmsh.model.occ.synchronize()

    # Physical groups
    gmsh.model.addPhysicalGroup(2, [disk], -1, "Cavity")
    gmsh.model.addPhysicalGroup(
        2, [x[1] for x in sc_dimtags if x[0] == 2], -1, "SC_boundary"
    )
    ...
    points = [
        x[1]
        for x in gmsh.model.getBoundary([(2, disk)], False, True, True)
        if x[0] == 0
    ]
    curves = [
        x[1]
        for x in gmsh.model.getBoundary([(2, disk)], False, False, False)
        if x[0] == 1
    ]
    ...
    gmsh.model.mesh.field.setNumbers(1, "PointsList", points)
    gmsh.model.mesh.field.setNumbers(1, "CurvesList", curves)
    gmsh.model.mesh.field.setNumber(1, "Sampling", radius / size_min)
    ...
    gmsh.model.mesh.field.setNumber(2, "SizeMax", size_max)
    gmsh.model.mesh.field.setNumber(2, "DistMin", 3 * size_min)
    gmsh.model.mesh.field.setNumber(2, "DistMax", 0.2 * radius)
